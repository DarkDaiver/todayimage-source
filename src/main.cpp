...
#if defined(NDEBUG)
# error "TodayImage cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa2f06b17742b9116c2d0d9e93aad30d58e248bfa7607c7e61cc43b39810418fd");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // TodayImage: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "TodayImage Signed Message:\n";
...
{
    // TodayImage: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // TodayImage: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // TodayImage: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1469821807, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04eac3b7c28b7ef2a5d0abab5d8f5b30479e38d89d9e346ceca33746a802f8a8dbfb2ead2f9a53cf93073e7b3d4672385cf4dc3c3e601c38dbb0461422b16391c2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1469821807;
        ...
        assert(block.hashMerkleRoot == uint256("0x2961544358954af403ce86aeeddff01b4ff7d3c19101d1dd0ae261c942c10279"));
...
unsigned char pchMessageStart[4] = { 0xba, 0xad, 0x07, 0x9c }; // TodayImage: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TodayImageMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TodayImageMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TodayImageMiner(CWallet *pwallet)
{
    printf("TodayImageMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("todayimage-miner");
    ...
    try { loop {
        ...
        printf("Running TodayImageMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TodayImageMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TodayImageMiner, pwallet));
}
