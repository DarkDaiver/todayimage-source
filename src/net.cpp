...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "TodayImage " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"104.238.185.61", "node.104.238.185.61"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. TodayImage is probably already running."), addrBind.ToString().c_str());
        else
